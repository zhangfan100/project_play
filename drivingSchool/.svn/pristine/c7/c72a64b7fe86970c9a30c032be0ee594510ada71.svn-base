...
<body style="overflow:hidden;">
		...
		<div data-options="region:'center',border:false" style="" >
			<div id="" class="easyui-layout" data-options="fit:true" >
				...
				<div data-options="region:'center',border:false" >
					<table id="comment_manage_dg" class="easyui-datagrid" title="" 
							...
							url:'${path }/mag/queryLessonReviewPage',method:'POST',onClickCell: onClickCell">
						<thead>
							<tr>
								<th data-options="field:'ck',checkbox:true,width:50"></th>
								<th data-options="field:'name',width:50,align:'center'">教练</th>
								<th data-options="field:'stuName',width:50,align:'center'">评论用户</th>
								<th data-options="field:'content',width:250,align:'center'">评论内容</th>
			...
			<div id="comment_window" class="easyui-window" title="评论回复" data-options="cache:false,modal:true,border:false,shadow:false,collapsible:false,minimizable:false,maximizable:false,closed:true,iconCls:'icon-save'" style="width:772px;height:315px;font-size:16px;font-weight:bold;color:#5d5c5c;">
				<form id="replySubmitForm">
					...
					<button id="comment_reply_button" name="" type="button" onclick="submitLessonReviewReply()">回复</button>
				</form>
			</div>
		</div>
	...
	<script type="text/javascript">
		//提交回复
		function submitLessonReviewReply(){
		...
		function formattrOperator(value,row,index){
			var text = row.isDisplay==0?'显示':'隐藏';
			var hideLessonReview = "<a class='comment_a_reply' rindex='"+index+"' href='javascript:void(0);' onclick='hideLessonReview(this)'>"+text+"</a>";
			return hideLessonReview+"&nbsp;&nbsp;<a class='comment_a_reply' href='javascript:reply("+row.id+")'>评论</a>";
		}
		
		function hideLessonReview(obj){
			var _this = $(obj);
			...
			$.post("${path}/mag/hideLessonReview",param,function(data){
		...
		function del(param){
			$.messager.confirm(GV.title, '确认删除?', function(r){
				if (r){
					$.post("${path}/mag/delLessonReview",param,function(data){
						switch(data*1){
