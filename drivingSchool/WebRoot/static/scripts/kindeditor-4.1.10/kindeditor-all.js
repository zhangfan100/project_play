	...
	_each(('cut,copy,paste').split(','), function(i, name) {
		self.clickToolbar(name, function() {
			...
			try {
				...
				AlterMessage(self.lang(name + 'Error'));
			}
		});
	});
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (url == 'http://' || K.invalidUrl(url)) {
							AlterMessage(self.lang('invalidUrl'));
							urlBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(width)) {
							AlterMessage(self.lang('invalidWidth'));
							widthBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(height)) {
							AlterMessage(self.lang('invalidHeight'));
							heightBox[0].focus();
							return;
						}
				...
				var uploadbutton = K.uploadbutton({
					...
					afterUpload : function(data) {
						...
						if (data.error === 0) {
							...
							AlterMessage(self.lang('uploadSuccess'));
						} else {
							AlterMessage(data.message);
						}
					},
				...
				click : function(e) {
					...
					if (showLocal && showRemote && tabs && tabs.selectedIndex === 1 || !showRemote) {
						if (uploadbutton.fileBox.val() == '') {
							AlterMessage(self.lang('pleaseSelectFile'));
							return;
						}
						dialog.showLoading(self.lang('uploadLoading'));
						uploadbutton.submit();
...
KindEditor.plugin('map', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		var iframeHtml = ['<!doctype html><html><head>',
			...
			'			});',
			'		} else {',
			'			AlterMessage("Invalid address: " + address);',
			'		}',
			'	});',
			'}',
			'</script>',
	...
	init : function(options) {
		...
		var settings = {
			...
			file_queue_error_handler : function(file, errorCode, message) {
				...
				K.DEBUG && AlterMessage(errorName);
			},
...
SWFUpload.Console.writeLine = function (message) {
	...
	try {
		...
		AlterMessage("Exception: " + ex.name + " Message: " + ex.message);
	}
};
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (rows == 0 || !/^\d+$/.test(rows)) {
							AlterMessage(self.lang('invalidRows'));
							rowsBox[0].focus();
							return;
						}
						...
						if (!/^\d*$/.test(padding)) {
							AlterMessage(self.lang('invalidPadding'));
							paddingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(spacing)) {
							AlterMessage(self.lang('invalidSpacing'));
							spacingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(border)) {
							AlterMessage(self.lang('invalidBorder'));
							borderBox[0].focus();
							return;
						}
						//modify table
